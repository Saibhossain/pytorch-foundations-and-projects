...
def get_checks_classifier(learner_model, dummy_flattened_size=128):
    ...
    # check nn.Linear calls and their parameters
    with patch("torch.nn.Linear", wraps=torch.nn.Linear) as mock_linear:
...
def create_mocks_and_run_function(learner_func):
    results = {}
    ...
    try:
        ...
        mock_param2.numel.return_value = 9999  # Should be ignored

        mock_param3 = Mock()
        mock_param3.requires_grad = True
        mock_param3.numel.return_value = 300
        ...
        expected = 100 + 300  # Only trainable params are included

        # --- Tests ---
        results["numeric_case"] = result == expected
        results["model.parameters()"] = mock_model.parameters.called
        results["param.numel()"] = mock_param1.numel.called
        # results['param2.numel()'] = not mock_param2.numel.called  # should NOT be called
        # results['param3.numel()'] = mock_param3.numel.called
