    ...
    def g():
        ...
        try:
            expected_flattened_size = (
                2048  # This is the expected flattened size after the feature extraction
            )
...
def exercise_2(learner_func):
    def g():
        ...
        if not isinstance(learner_func, FunctionType):
            t.failed = True
            t.msg = "create_full_dataset has incorrect type"
            t.want = FunctionType
...
def exercise_4(learner_func):
    def g():
        ...
        try:
            checks = unittests_utils.create_mocks_and_run_function(learner_func)
            # print(checks)  # Uncomment for debugging

            # region == A - behavioural checks ==
            for expression, expected_call in checks.items():
                if expression != "numeric_case":
                    t = test_case()
                    if not expected_call:
                        t.failed = True
                        if expression == "model.parameters()":
                            t.msg = "model_parameters is not completed correctly"
                            t.want = "Expected call to model.parameters() to obtain model parameters"
                            t.got = "model.parameters() is not present"
                            cases.append(t)
                        elif expression == "param.numel()":
                            t.msg = (
                                "total_trainable_parameters is not completed correctly"
                            )
                            t.want = "Expected call to param.numel() to obtain the number of elements for each param."
                            t.got = "param.numel() is not present"
                            cases.append(t)
                        return cases
            # endregion ==

            # region == B - functional checks ==
            numerical_check = checks["numeric_case"]
            t = test_case()
            if not numerical_check:
                t.failed = True
                t.msg = "Numerical check failed. Check that the expression used to update the total_trainable_parameters is correct."
                t.want = "Numerical check to pass."
                t.got = "The total_trainable_parameters is not calculated correctly."
            cases.append(t)
        # endregion ==
